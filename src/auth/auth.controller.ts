...
import { Body, Controller, Delete, Get, Param, Post, Headers, HttpCode } from '@nestjs/common';
...
export class AuthController {
  ...
  @ApiResponse({ status: 201, description: 'Login successful — returns user details and JWT token.' })
  ...
  @ApiResponse({ status: 201, description: 'Device logout successful.' })
async deviceLogout(@Body() logoutDto: DeviceLogoutDto) {
  ...
  @ApiResponse({ status: 201, description: 'Deletion request received.' })
...
@Public()
  ...
  @ApiResponse({ status: 201, description: 'Deletion request status returned.' })
